...
async function dbStatus() {
  try {
    ...
    console.log(
      `${PASS} DB connection: OK (${challenges} challenges, ${users} users, ${progress} progress records, ${streaks} streaks)`
    )
    return true
  } catch (err) {
...
async function challengesList() {
  ...
  for (const c of challenges) {
    console.log(
      `  [${c.order}] ${c.id}  "${c.title}"  ${c.difficulty}  ${c.category}  ${c.xpReward}xp`
    )
  }
  return challenges
}
...
async function challengesTestExecutor() {
  ...
  for (const c of challenges) {
    ...
    if (result.passed) {
      console.log(
        `${PASS} Challenge "${c.title}": solution passes ${result.passedTests}/${result.totalTests} tests (${Math.round(result.executionTimeMs)}ms)`
      )
    } else {
      allPassed = false
      console.log(
        `${FAIL} Challenge "${c.title}": ${result.passedTests}/${result.totalTests} tests passed`
      )
...
async function progressSubmit(userId: string, challengeId: string, codeFile: string) {
  ...
  if (execution.passed) {
    ...
    if (existingProgress?.status === "completed") {
      console.log(
        `${PASS} Solution passes all tests but challenge already completed — no XP awarded`
      )
      return true
    }
    ...
    console.log(
      `${PASS} Progress: submitted solution for "${challenge.title}" — ${execution.passedTests}/${execution.totalTests} passed, +${xpAwarded} XP`
    )
  } else {
    const existingProgress = await prisma.userProgress.findUnique({
      where: { userId_challengeId: { userId, challengeId } },
    ...
    console.log(
      `${FAIL} Progress: solution failed — ${execution.passedTests}/${execution.totalTests} passed (attempt #${currentAttempts})`
    )
    for (const r of execution.results) {
      if (!r.passed) {
        console.log(
          `    ${r.description}: ${r.error ?? `expected ${JSON.stringify(r.expected)}, got ${JSON.stringify(r.actual)}`}`
        )
      }
    }
  }
...
async function progressStatus(userId: string) {
  ...
  for (const c of challenges) {
    ...
    if (!p) {
      ...
      console.log(
        `  ${green("[✓]")} ${c.title}  ${green("completed")}  +${p.xpEarned}XP  ${p.attempts} attempt(s)`
      )
    } else {
      console.log(
        `  ${yellow("[~]")} ${c.title}  ${yellow("in progress")}  ${p.attempts} attempt(s)`
      )
    }
  }
}
...
async function streakCheck(userId: string) {
  ...
  if (isBroken) {
    console.log(
      `  ${red("Streak is broken!")} ${streak.freezesLeft > 0 ? "(freeze available)" : "(no freezes)"}`
    )
  }
}
...
async function streakUpdate(userId: string) {
  ...
  console.log(
    `${PASS} Streak: updated (current: ${updated.currentStreak}, longest: ${updated.longestStreak}, freezes: ${updated.freezesLeft})`
  )
  return updated
}
...
async function verifyAll() {
  ...
  if (testUser) {
    ...
    const isBroken =
      afterBreak!.lastActiveDate !== null &&
      !isActiveToday &&
      !wasActiveYesterday &&
      afterBreak!.freezesLeft === 0
