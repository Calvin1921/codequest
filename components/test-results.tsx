    ...
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        {xpAwarded !== undefined && xpAwarded > 0 && (
          <span className="font-mono text-sm text-lime-500 tabular-nums">+{xpAwarded} XP</span>
        )}
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        result.passed ? "border-neutral-800 bg-neutral-900/50" : "border-red-500/20 bg-red-500/5"
      )}
    >
      <button
        type="button"
      ...
      >
        ...
        <span
          className={cn("flex-1 font-medium", result.passed ? "text-neutral-300" : "text-red-300")}
        >
          {result.description}
        </span>
        <span className="font-mono text-xs text-neutral-600">{result.executionTimeMs}ms</span>
        {!result.passed && (
          <svg
            className={cn("size-4 text-neutral-500 transition-transform", expanded && "rotate-180")}
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
      ...
      {!result.passed && expanded && (
        <div className="border-t border-red-500/10 px-3 pt-2 pb-3">
